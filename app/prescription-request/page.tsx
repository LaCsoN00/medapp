...
import { getPrescriptionRequests, getPatientByUserId } from "@/actions";
...
interface PrescriptionRequest {
  ...
  medecin: { firstName: string; lastName: string; speciality?: { name: string } };
...
export default function PrescriptionRequestListPage() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        ...
        if (!patient) return;
        const reqs = await getPrescriptionRequests({ patientId: patient.id });
  ...
  return (
    <section className="section-padding bg-base-100 min-h-screen w-full">
